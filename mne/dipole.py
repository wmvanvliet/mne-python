...
class DipoleFitter:
    ...
    def __init__(
        ...
        *,
        ...
        self._whitener, _, self.rank = compute_whitener(
            cov, info, picks=self._picks, rank=rank, return_rank=True
        )
    ...
    def init_grid(self, *, guess_grid=0.02, guess_mindist=None, guess_exclude=0.02):
        ...
        # if fixed_position:
        #     if "rr" in inner_skull:
        #         check = _surface_constraint(pos, inner_skull, self._min_dist_to_inner_skull)
        #     else:
        #         check = _sphere_constraint(
        #             pos,
        #             inner_skull["r0"],
        #             R_adj=inner_skull["R"] - self._min_dist_to_inner_skull,
        #         )
        #     if check <= 0:
        #         raise ValueError(
        #             f"fixed position is {-1000 * check:0.1f}mm outside the inner skull "
    ...
    def fit(self, evoked, *, pos=None, ori=None):
        """Fit a dipole.
        ...
        # Do actual fits.
        data = data[self._picks]
        ch_names = [info["ch_names"][p] for p in self._picks]
        proj_op = make_projector(info["projs"], ch_names, info["bads"])[0]
        # fun = _fit_dipole_fixed if fixed_position else _fit_dipole
        fun = _fit_dipole
        out = _fit_dipoles(
        ...
        residual = evoked.copy().apply_proj()  # set the projs active
        residual.data[self._picks] = np.dot(proj_op, out[-1])
        logger.info("%d time points fitted", len(dipoles.times))
        return dipoles, residual
